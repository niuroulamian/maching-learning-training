...
# 设计特征
# -----------------------------
def extract_struct_features_no_votes(df: pd.DataFrame) -> np.ndarray:
    """
    （无 votes 版本）结构化特征提取：
    - 刻意去掉 votes_up / votes_all / helpful_ratio
    - 目的：避免“用投票结果预测评论质量”导致标签泄漏
    """
    text = df["reviewText"].fillna("").astype(str)

    # 评论长度相关
    len_chars = text.str.len().astype(float)
    words = text.str.split()
    len_words = words.str.len().fillna(0).astype(float)
    avg_word_len = (len_chars / (len_words + 1.0)).astype(float)

    # 标点符号/格式相关
    num_exclaim = text.str.count("!").astype(float)
    num_question = text.str.count(r"\?").astype(float)

    # 大写字母占比：用于捕捉“吼叫式”表达、强调式写法
    ...
    ratio_upper = text.apply(upper_ratio).astype(float)

    # 星级评分：保留（它不是“帮助投票”，相对安全）
    overall = df["overall"].fillna(df["overall"].median()).astype(float)

    # 注意：这里不再加入 votes_all / helpful_ratio
    feats = np.vstack([
        len_chars.values,
        len_words.values,
        avg_word_len.values,
...
def build_features(
    ...
    F_tr = extract_struct_features_no_votes(train_df)
    F_te = extract_struct_features_no_votes(test_df)
...
class BaggingEnsemble:
    ...
    def fit(self, X, y):
        ...
        for _ in tqdm(
            ...
            model.fit(X, y, sample_weight=D) # 用sample weight， 避免巨大稀疏矩阵拷贝, 速度极慢
...
def main():
    ...
    # 2) 仅结构化（给树用）
    F_train = extract_struct_features_no_votes(train_df)
    F_test  = extract_struct_features_no_votes(test_df)
    ...
    base_svm_bag = LinearSVC(C=0.5, max_iter=20000) # 跑的过程出现警告，加大迭代次数
    ...
    for name, model in models.items():
        if name in {"AdaBoost_DT", "Bagging_DT"}:
            X_train_used = csr_matrix(F_train)
            X_test_used = csr_matrix(F_test)
        else:
            X_train_used = X_train
            X_test_used = X_test

        auc, fitted_model = auc_on_group_split(X_train_used, y,groups, model, val_ratio=0.15, seed=42)
        results.append((name, auc))

        # fit on full training and predict test
        fitted_model.fit(X_train_used, y)
        test_scores = fitted_model.predict_score(X_test_used)
